...
function search_company(search_period){	
    ...
    formObj.method="get";
    formObj.action="/AA/admin/company/:.do";
    formObj.submit();
}
	 ...
	 <c:otherwise>
	     <c:forEach var="item" items="${company_list}" >
	            <tr >       
					...
					<td width=10% >
					  <strong>${item.company_phone}</strong><br>
					</td>
	  <td width=50%>
            <c:choose>
                <c:when test="${empty item.company_roadAdd}">
                    <strong>등록 주소 없음</strong><br>
                </c:when>
                <c:otherwise>
                    <strong>${item.company_roadAdd}</strong><br>
                </c:otherwise>
            </c:choose>
            <strong>${item.company_jibunAdd}</strong><br>
            <strong>${item.company_namujiAdd}</strong><br>
        </td>
					<td width=10%>
				    ...
				    <td width=10% >
					  <strong>${item.totalWork}</strong><br>
					</td>
				</tr>
		</c:forEach>
	</c:otherwise>
